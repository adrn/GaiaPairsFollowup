...
as $8~\pc$ have been discovered in the solar neighborhood using HIPPARCOS.
% Aims
...
For example, they have been used to place limits on the mass distribution and
population of massive perturbers in the Galaxy (e.g., \citealt{Yoo:2004}), to
study the dynamical outcomes of star formation processes (e.g.,
\citealt{Reipurth:2012}), and to calibrate stellar models and inferences across
the main sequence (e.g., \citealt{Brewer:2016}).
...
bound wide binaries and are therefore coeval.
At larger separations ($\gtrsim 0.1~\pc$), comoving pairs are likely a mix of
tenuously bound stars, recently disrupted binaries, moving groups or dynamical
features, and the final products of dissolving stellar associations or open
clusters.
...
unbound systems collectively as ``comoving stars.''

Comoving stars are identified by searching for approximately cospatial stars
that appear to be comoving either in full-space or in sky-projected velocities.
...
surveys (\citealt{Steinmetz:2006}), several hundred confirmed comoving pairs
(\citealt{Shaya:2011}) and over 10,000 candidate systems
(\citealt{Gould:2004,Lepine:2007,Tokovinin:2012,Allen:2014,Oh:2017,Oelkers:2017,
Andrews:2017}) have already been discovered.
Radial velocity follow-up is still needed to confirm the majority of these
pairs, though \gaia\ \DR{2} will provide radial velocities for the brightest
stars ($G < 12$; \citealt{Gaia-Collaboration:2016}).

It has long been recognized that the observed period or separation distribution
of binary stars encapsulates information about star formation and dynamical
processes of the Galaxy (\citealt{Opik:1924}).
[If the initial distribution power law, break and steepens from perturbations]
[a bit of theory about wide binaries...predictions for wide binary separations?]
(Weinberg87)

Using candidate comoving pairs selected based on their proper motions alone,
several groups have measured the separation distribution of comoving stars at
angular separations $\Delta \theta \lesssim 1^\circ$ where the false-positive
rate is expected to be lower (\citealt{Chaname:2004,Lepine:2007,Sesar:2008});
these are likely predominantly wide binaries.
...
0.65~\pc$ at a distance of $150~\pc$).
...
While the separation distribution of wide binaries appears to and is expected to
fall off rapidly with increasing separation, predictions for the separation
distribution of \emph{comoving stars} are less clear.
Early work suggested ionized pairs don't matter much...
[But including tidal field...] (jiang and tremaine)
[But still haven't included perturbers...]
[Tentative evidence of this population in (shaya and olling)]
[Need larger samples of confirmed large-separation pairs]
...
calibration targets.
For the purposes of this work, all of the calibration targets have previously measured radial velocities and were selected from high-resolution spectroscopic
...
\todo{Highlight the G subgiant-A pair}


\subsubsection{HD 151090 \& HIP 81988}

\todo{G-subgiant - K-dwarf - well known high proper-motion comoving pair}


\subsubsection{HD 54318 \& TYC 3792-658-1}

\todo{Highlight the largest mag difference MS pair - K-dwarf - B-star}
...
% \begin{figure}[htbp]
%   \begin{center}
%     \includegraphics[width=\linewidth]{genuine_cmd_rave.pdf}
%   \end{center}
%   \caption{%
%     Color-magnitude diagram of \emph{genuine} comoving star pairs plotted over
%     the density of all \tgas\ sources within 200 pc; similar to
