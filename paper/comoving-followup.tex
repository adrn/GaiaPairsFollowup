...
We model the absorption line using a pixel-convolved Voigt profile and again use
a Gaussian process for the background model (with the same parameterizations as
described above).
...
probability distribution over the 6 parameters: $\ln |A|$, $x_0$, $\ln\sigma_G$,
$\ln\gamma_L$, $\ln\sigma_{3/2}$, and $\ln\rho_{3/2}$.
...
\todo{Explain MCMC, emcee, steps, etc.}
\figurename~\ref{fig:Halpha-mcmc} shows \todo{mcmc shite}.
...
We use the MAD to estimate the standard deviation ($\sigma \approx 1.5~{\rm
  ...
  \begin{center}
    \begin{tabular}{ c | c }
      \toprule
        ...
        $\ln A$ & $\mathcal{U}(2, 16)$\\
        $x_0$ & $\mathcal{U}(\lambda_{\Ha}-16, \lambda_{\Ha}+16)$ [${\rm \AA}$]\\
        $\ln\sigma_G$ & $\mathcal{U}(-8, 2)$ [${\rm \AA}$]\\
        $\ln\gamma_L$ & $\mathcal{U}(-8, 2)$ [${\rm \AA}$]\\
        $\ln\sigma_{3/2}$ & $\mathcal{U}(-8, 8)$\\
        $\ln\rho_{3/2}$ & $\mathcal{N}(1, 1)$ [${\rm \AA}$]\\
      \bottomrule
    \end{tabular}
    \caption{
      ...
      $\lambda_{\Ha}$ is the true \Ha\ line centroid in angstroms.
