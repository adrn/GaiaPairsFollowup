...
\subsection{Validation of absolute radial velocities}
...
From this distribution, we compute the median absolute deviation (MAD) and
estimate the standard deviation to be $\approx 20~\kms$, which we adopt as our
global systematic error for all absolute radial velocities.
...
Of the 323 observed star pairs, 11 have at least one member with an unreliable
(flagged) radial velocity measurement from the pipeline above.
For the remaining 312 pairs, we use the measured radial velocities to re-compute
...
\llrnew.
We consider a pair as being genuinely comoving if $\llrnew > \llrold$; 125 of
the 312 comoving pairs satisfy this constraint.
\figurename~\ref{fig:llr}

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=\linewidth]{llr-vs-dv.pdf}
  \end{center}
  \caption{%
    FML
    \label{fig:llr}}
...
TODO: check on ipython notebook re-running with updated observation shite

\subsection{Physical properties of genuine comoving pairs}\label{sec:}

\todo{Plot of magnitude of 3d velocity difference vs. 3d separation?}
